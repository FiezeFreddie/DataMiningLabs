...
public class Cluster extends ArrayList<FeatureVector> {
	...
	public FeatureVector centroid() {
		...
		// add code here
		if (changed) {
		    centroid = new FeatureVector();
		    Double[] temp = new Double[this.get(0).size()];
	...
	public double meanDistanceTo(Cluster other) {
        ...
        FeatureVector otherCentroid = other.centroid;

        for (int i = 0; i < centroid.size(); i++){
            dist += (centroid.get(i) - otherCentroid.get(i)) * (centroid.get(i) - otherCentroid.get(i));
        }
	...
	public List<Double> distancesToCluster(Cluster other) {
		...
		// add code here
		for (FeatureVector fv : this){
		    for (FeatureVector ofv : other){
                double dist = 0d;
		        for (int i = 0; i < fv.size(); i++){
		            dist += (fv.get(i) - ofv.get(i)) * (fv.get(i) - ofv.get(i));
                }
                result.add(dist / fv.size());
            }
        }
		return result;
	}
	...
	public double calculateAverageRSS() {		
		...
		for (FeatureVector fv : this){
            double dist = 0d;
		    for (int i = 0; i < fv.size(); i++){
		        dist += (fv.get(i) - centroid.get(i)) * (fv.get(i) - centroid.get(i));
            }
            rss += dist;
        }
		return rss / size();
