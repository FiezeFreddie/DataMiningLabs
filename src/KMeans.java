...
public class KMeans {
	...
	private void addInitPoint() {
		double rand = Math.random() * getData().size();
		Cluster c = getData();
		FeatureVector randPoint = c.get((int)Math.floor(rand));
		Cluster newClust = new Cluster();
		newClust.add(randPoint);
		for (Cluster cl : clusters ) {
            if (cl.get(0).contains(randPoint)) {
                return;
            }
        }
	...
	public void update() {
        ...
        ArrayList<FeatureVector> centStorage = new ArrayList<>();

	    for (int i = 0; i < clusters.size(); i++){
	        centStorage.add(clusters.get(i).centroid());
	        clusters.get(i).clear();
        }

        for (FeatureVector fv: getData()){
	        double minDist = Integer.MAX_VALUE;
	        int minDex = 0;
            for (int j = 0; j < centStorage.size(); j++) {
                double dist = 0d;
                for (int i = 0; i < fv.size(); i++) {
                     dist += (fv.get(i) - centStorage.get(j).get(i)) * (fv.get(i) - centStorage.get(j).get(i));
                }
